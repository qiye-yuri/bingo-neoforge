...
设置书的“对局参数”页还可以按分钟调整计时模式时长，并按区块调整队伍复活点间距。各队出生与死亡复活位置以 `(0,0)` 为中心呈网格分布；比赛结束后才恢复大厅复活点。运行时下界会被识别为原版下界的等价实例，因此黑曜石传送门可以创建本局下界出口并正常往返。

命令开局时，服务器会在不重启的情况下创建本局独占的主世界、下界和末地实例，并把参赛玩家传送到由 Bingo 种子确定的新区域。下界与末地传送门会被路由到本局世界组；结束游戏、完成胜利条件或排位倒计时结束后，玩家会返回大厅，三个比赛世界将保存并从运行中的服务器卸载。
