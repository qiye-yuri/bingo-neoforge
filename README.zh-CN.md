...
设置书还提供全员夜视、死亡不掉落和队伍共享箱开关。队伍箱为每队独立的 54 格共享库存，队员可使用 `/neobingo teamchest` 或书内按钮共同访问；箱内物品仅保留到本局结束。初始物资页可分别增减面包、原木、圆石、火把、铁锭和熟牛肉，开局时按配置发给每名参赛玩家。若原版传送计时仍未触发，站在传送门内 5 秒后会执行维度切换兜底。
