...
设置书不再使用固定难度预设。MAX、S、A、B、C、D 六档数量集中在同一页并可逐个增减；书中显示当前数量，安装客户端模组后点击调整会在当前书页原位更新数值，无需关闭重开。仅安装服务端模组的原版客户端仍可在屏幕下方和聊天栏看到更新结果，重新打开书也会显示新数值。六档合计为 25 时，可在大厅刷新即将开始的棋盘，并使用这张棋盘直接开局。游戏开始后刷新入口会被服务器拒绝，独立的完整棋盘查看界面和 `/neobingo card` 命令已移除。
