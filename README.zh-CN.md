...
设置书的“对局参数”页还可以按分钟调整计时模式时长，并按区块调整队伍复活点间距。各队出生与死亡复活位置以 `(0,0)` 为中心呈网格分布；比赛结束后才恢复大厅复活点。比赛改为直接使用服务器持续加载的原版三维度，从根本上兼容黑曜石下界传送门。

设置书还提供全员夜视、死亡不掉落和队伍共享箱开关。队伍箱为每队独立的共享库存，可在书中调整为 1–6 行；队员可使用 `/neobingo teamchest` 或书内按钮共同访问，箱内物品会参与 Bingo 自动判定，并在下一局开始时清空。初始物资页可分别增减面包、原木、圆石、火把、铁锭和熟牛肉，开局时按配置发给每名参赛玩家。若原版传送计时仍未触发，站在传送门内 5 秒后会执行维度切换兜底。

命令开局不需要重启服务器。参赛玩家会进入以 `(0,0)` 为中心的比赛区域，并使用服务器原版主世界、下界和末地；结束游戏、完成胜利条件或排位倒计时结束后，玩家会返回大厅。
