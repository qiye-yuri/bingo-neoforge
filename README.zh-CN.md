...
设置书还提供全员夜视、死亡不掉落和队伍共享箱开关。队伍箱为每队独立的共享库存，可在书中调整为 1–6 行；安装客户端模组后默认按 `G` 打开，也可使用 `/neobingo teamchest` 或书内按钮，按键可在控制设置中重新绑定。箱内物品会参与 Bingo 自动判定，并在下一局开始时清空。初始物资页可对主手持有的任意注册物品自由增减，也可清空全部配置；常用物品仍保留快捷按钮，开局时按配置发给每名参赛玩家。若原版传送计时仍未触发，站在传送门内 5 秒后会执行维度切换兜底。
