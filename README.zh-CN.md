...
设置书不再使用固定难度预设。MAX、S、A、B、C、D 六档数量集中在同一页并可逐个增减；书中显示当前数量，调整后屏幕下方立即显示设置摘要，重新打开书即可看到更新后的数值。六档合计为 25 时，可在大厅刷新即将开始的棋盘，并使用这张棋盘直接开局。游戏开始后刷新入口会被服务器拒绝，独立的完整棋盘查看界面和 `/neobingo card` 命令已移除。
